...
OUTPUT_FILE = 'panchayat_samiti_member.csv'
...
class BiharScraper:
    ...
    def GetPradesikList(self, post_value, district_value, blok_value):
        # set params
        params = {
            '__EVENTARGUMENT': '',
    ...
    def GetReservationStatus(self, post_value, district_value, blok_value, pradesik_value):
        # set params
        params = {
            '__EVENTARGUMENT': '',
    ...
    def GetBiharList(self, post, district, blok, pradesik, reservation_status):
    ...
    def Start(self,
        ...
        # write header into output csv file
        if start_district == '' and start_blok == '' and start_pradesik == '': self.WriteHeader()
        ...
        for post in post_list:
            ...
            for district in district_list:
                ...
                for blok in blok_list:
                    ...
                    # get pradesik list
                    print('getting pradesik list for %s:%s:%s...' % (post['name'], district['name'], blok['name']))
                    pradesik_list = self.GetPradesikList(post['value'], district['value'], blok['value'])
                    print(pradesik_list)
                    ...
                    for pradesik in pradesik_list:
                        ...
                        print('getting reservation status for %s:%s:%s%s...' % (post['name'], district['name'], blok['name'], pradesik['name']))
                        reservation_status = self.GetReservationStatus(post['value'], district['value'], blok['value'], pradesik['value'])
                        print(reservation_status)

                        # get bihar list
                        print('getting bihar list for %s:%s:%s%s...' % (post['name'], district['name'], blok['name'], pradesik['name']))
                        ret_val = self.GetBiharList(post, district, blok, pradesik, reservation_status)
                        print(ret_val)
...
def main():
    ...
    # start to scrape
    scraper.Start(
        start_post='Panchayat Samiti Member',
        start_district='',
        start_blok='',
        start_pradesik=''
    )
