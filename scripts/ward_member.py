...
class BiharScraper:
    ...
    def GetWardList(self, post_value, district_value, blok_value, panchayat_value):
        # set params
        params = {
            '__EVENTARGUMENT': '',
            '__EVENTTARGET': 'ddlBlok',
        ...
        if ret.status_code == 200:
            ...
            # get panchayat list
            options = Selector(text=ret.text).xpath('//select[@id="ddlWard"]/option').extract()
            ...
            print('fail to get ward list')

    def GetPradesikAndReservationStatus(self, post_value, district_value, blok_value, panchayat_value, ward_value):
        # set params
        params = {
            '__EVENTARGUMENT': '',
    ...
    def GetBiharList(self, post, district, blok, panchayat, ward, pradesik, reservation_status):
        self.bihar_form_data = {
            '__EVENTVALIDATION': self.ward_form_data['__EVENTVALIDATION'],
            '__VIEWSTATE': self.ward_form_data['__VIEWSTATE'],
            '__VIEWSTATEGENERATOR':self.ward_form_data['__VIEWSTATEGENERATOR']
        }
    ...
    def Start(self,
        ...
        # write header into output csv file
        if start_district == '' and start_blok == '' and start_panchayat == '' and start_ward == '' and start_pradesik == '': self.WriteHeader()
            ...
            for district in district_list:
                ...
                for blok in blok_list:
                    ...
                    for panchayat in panchayat_list:
                        ...
                        # get ward list
                        print('getting ward list for %s:%s:%s:%s...' % (post['name'], district['name'], blok['name'], panchayat['name']))
                        ward_list = self.GetWardList(post['value'], district['value'], blok['value'], panchayat['value'])
                        print(ward_list)

                        for ward in ward_list:
                            if start_ward == ward['name']: ward_flag = True
                            if ward_flag == False: continue

                            # get pradesik and reservation status
                            print('getting pradesik and reservation status for %s:%s:%s:%s:%s...' % (post['name'], district['name'], blok['name'], panchayat['name'], ward['name']))
                            ret_val = self.GetPradesikAndReservationStatus(post['value'], district['value'], blok['value'], panchayat['value'], ward['value'])
                            print(ret_val)
                            ...
                            # get bihar list
                            print('getting bihar list for %s:%s:%s%s:%s:%s...' % (post['name'], district['name'], blok['name'], panchayat['name'], pradesik['name'], ward['name']))
                            ret_val = self.GetBiharList(post, district, blok, panchayat, ward, pradesik, reservation_status)
                            print(ret_val)
...
def main():
    ...
    # start to scrape
    scraper.Start(
        start_post='Ward Member',
        start_district='22 - BHAGALPUR',
        start_blok='009 - Pirpainti',
        start_panchayat='06 - Shrimatpur Hujurnagar',
        start_ward='12 - topra tola',
        start_pradesik=''
    )
